    ...
    data() {
      return {
        ...
        getBuscar:async function(intPagina=1,strTipo = ""){
            ...
            const response = await fetch(base_url+"/Configuracion/Administracion//getBuscar",{method:"POST",body:formData});
        ...
        getDatos:function(id,tipo=1){
            if(tipo==1){
                window.location.href=base_url+'/email/message/'+id
            }else{
                window.location.href=base_url+'/email/sent/'+id
            }
        },
        delDatos:function(intId,strTipo){
            ...
            Swal.fire({
              ...
              if(result.isConfirmed){
                  ...
                  formData.append("tipo_busqueda",strTipo);
                  const response = await fetch(base_url+"/Configuracion/Administracion/delDatos",{method:"POST",body:formData});
                  const objData = await response.json();
                  if(objData.status){
        ...
        getMensaje:async function(){
            ...
            const response = await fetch(base_url+"/Configuracion/Administracion/getMensaje",{method:"POST",body:formData});
            const objData = await response.json();
        ...
        setMensaje:async function(){
            if(this.intTipoPagina == 3){
                ...
                if(this.strMensaje ==""){
                    Swal.fire("Error", "You must write the message!", "error");
                    return false;
                }
            }
            ...
            const response = await fetch(base_url+"/Configuracion/Administracion/setMensaje",{method:"POST",body:formData});
            const objData = await response.json();
            this.$refs.btnAdd.innerHTML = `Save <i class="fas fa-paper-plane"></i>`;
