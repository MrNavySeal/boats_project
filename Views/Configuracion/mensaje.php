...
<div class="body flex-grow-1 px-3" id="<?=$data['page_name']?>">
    <h2 class="fs-5">Subject: {{objMensaje.id !="" ? objMensaje.asunto : ""}}</h2>
    <p></p>
    <div class="d-flex justify-content-between flex-wrap">
        <div>
            <p class="m-0"><strong>From: </strong>{{objMensaje.id !="" ? objMensaje.name+" <"+objMensaje.email+">" : ""}}</p>
            <p class="m-0"><strong>Phone: </strong>{{objMensaje.id !="" ? objMensaje.telefono : ""}}</p>
            <p class="m-0"><strong>Address: </strong>{{objMensaje.id !="" ? objMensaje.address : ""}}</p>
        </div>
