...
<div class="d-flex justify-content-end mb-3 flex-wrap gap-2">
    <?php if($data['botones']['atras']['mostrar']) { ?>
        <button type="button" class="btn btn-primary " <?=$data['botones']['atras']['evento']."=".'"'.$data['botones']['atras']['funcion'].'"'?>><?= $atras != "" ? $atras : "Back"?> <i class="fas fa-reply"></i></button>
    <?php }?>
    <?php if($data['botones']['buscar']['mostrar']) { ?>
        <button type="button" class="btn btn-primary " <?=$data['botones']['buscar']['evento']."=".'"'.$data['botones']['buscar']['funcion'].'"'?>><?= $buscar != "" ? $buscar : "Search"?> <i class="fas fa-search"></i></button>
    <?php }?>
    ...
    <?php if($data['botones']['duplicar']['mostrar']) { ?>
        <button type="button" class="btn btn-primary " <?=$data['botones']['duplicar']['evento']."=".'"'.$data['botones']['duplicar']['funcion'].'"'?>><?= $duplicar != "" ? $duplicar : "New window"?> <i class="fas fa-window-restore"></i></button>
    <?php }?>
    <?php if($data['botones']['nuevo']['mostrar']) { ?>
        <button type="button" class="btn btn-primary " <?=$data['botones']['nuevo']['evento']."=".'"'.$data['botones']['nuevo']['funcion'].'"'?>><?= $nuevo != "" ? $nuevo : "New"?> <i class="fas fa-plus"></i></button>
    <?php }?>
    <?php if($data['botones']['guardar']['mostrar']) { ?>
        <button type="button" class="btn btn-primary " <?=$data['botones']['guardar']['evento']."=".'"'.$data['botones']['guardar']['funcion'].'"'?>><?= $guardar != "" ? $guardar : "Save"?> <i class="fas fa-save"></i></button>
    <?php }?>
</div>